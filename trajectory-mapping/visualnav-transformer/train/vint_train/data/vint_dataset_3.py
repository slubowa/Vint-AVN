...
class ViNT_Dataset(Dataset):
    ...
    def _compute_scene_speed(self, traj_data):
        """
        Estimates scene-wise speed based on position changes.
        Returns an estimated speed per step.
        """
        positions = np.array(traj_data["position"])
        distances = np.linalg.norm(np.diff(positions, axis=0), axis=1)
        avg_speed = np.mean(distances)  # Average speed across trajectory
        return max(avg_speed, 0.01)  # Prevent zero speed
    ...
    def _build_index(self, use_tqdm: bool = False):
        ...
        for traj_name in tqdm.tqdm(self.traj_names, disable=not use_tqdm, dynamic_ncols=True):
            ...
            scene_speed = self._compute_scene_speed(traj_data)
            dynamic_spacing = max(1, int(scene_speed / self.data_config["metric_waypoint_spacing"]))  # Adjust spacing
            begin_time = self.context_size * dynamic_spacing
            end_time = traj_len - self.end_slack - self.len_traj_pred * dynamic_spacing
    ...
    def _load_image(self, trajectory_name, time):
        ...
        try:
            ...
            return None  # This might be causing the DataLoader issue

    def _compute_actions(self, traj_data, curr_time, goal_time):
        start_index = curr_time
        scene_speed = self._compute_scene_speed(traj_data)
        dynamic_spacing = max(1, int(scene_speed / self.data_config["metric_waypoint_spacing"]))  
        end_index = curr_time + self.len_traj_pred * dynamic_spacing + 1
        yaw = traj_data["yaw"][start_index:end_index:dynamic_spacing]
        positions = np.array(traj_data["position"][start_index:end_index:dynamic_spacing])
