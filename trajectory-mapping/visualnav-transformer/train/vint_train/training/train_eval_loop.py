...
def train_eval_loop(
    ...
    for epoch in range(current_epoch, current_epoch + epochs):
        ...
        for dataset_type in test_dataloaders:
            ...
            if config.get("early_stopping", False):  # <-- wrap early stopping in a config flag
                monitor_metric = config.get("scheduler_monitor", "total_loss")  # "total_loss" or "dist_loss"
                if monitor_metric == "dist_loss":
                    monitor_value = np.mean(test_dist_loss)
                else:
                    monitor_value = np.mean(avg_total_test_loss)

                if epoch == current_epoch:
                    best_loss = monitor_value
                    patience_counter = 0
                else:
                    if monitor_value < best_loss -0.5:  # min_delta = 0.5
                        best_loss = monitor_value
                        patience_counter = 0
                    else:
                        patience_counter += 1
                        print(f"EarlyStopping: patience {patience_counter}/{config.get('early_stopping_patience', 10)}")
